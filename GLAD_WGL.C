...
static void* APIENTRY get_proc(const char *namez);
...
static
void* APIENTRY get_proc(const char *namez) {
    void* result = NULL;
...
int __cdecl gladLoadWGL(HDC hdc) {
    int status = 0;
...
void __cdecl gladUnloadWGL(void) {
    close_wgl();
}
...
int __cdecl gladLoadWGLLoader(GLADloadproc load, HDC hdc) {
...
#if defined(_WIN32) && defined(GLAD_GLAPI_EXPORT)
